...
function MouseHighlighter.Create(mouseUse)
	...
	function UpdatePosition(position)
		...
		if startPos ~= nil then
			local lowVec2 = CellCenterToWorld(c, startPos.X, startPos.Y - 1, startPos.Z)
			local highVec2 = CellCenterToWorld(c, startPos.X, startPos.Y + 1, startPos.Z)
...
function onClickedUp(mouse)
	if on then
		...
		local minX = math.min(startPos.X, x)
		local minY = math.min(startPos.Y, y)
		local minZ = math.min(startPos.Z, z)

		local maxX = math.max(startPos.X, x)
		local maxY = math.max(startPos.Y, y)
		local maxZ = math.max(startPos.Z, z)
